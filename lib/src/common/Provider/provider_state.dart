...
class FavouriteItem with ChangeNotifier {
  final List<ItemModel> _itemListOne = [
    ...
    const ItemModel(productId: '2', productName: 'Dry Fruits', productDescription: '250gm, Price', productThumbNail: 'assets/Fruits/driFruits.png', unitPrice: '\$3.99'),
  ];  final List<ItemModel> _itemListTwo = [

    const ItemModel(productId: '3', productName: 'Bell Pepper Red', productDescription: '2kg, Price', productThumbNail: 'assets/Fruits/Shemla.png', unitPrice:'\$5.99'),
    const ItemModel(productId: '4', productName: 'Ginger', productDescription: '500gm, Price', productThumbNail: 'assets/Fruits/pngfuel 3.png', unitPrice:'\$6.99'),
  ]; final List<ItemModel> _itemListThree = [

    const ItemModel(productId: '5', productName: 'Beef Bone', productDescription: '2kg, Price', productThumbNail: 'assets/Fruits/pngfuel 4.png', unitPrice:'\$7.99'),
    const ItemModel(productId: '6', productName: 'Broiler Chicken', productDescription: '1kg, Price', productThumbNail: 'assets/Fruits/pngfuel 5.png', unitPrice:'\$8.99'),
  ];
  ...
  final List<ItemModel> _selectedFavourites = []; // Track selected favourite items
  ...
  void favouriteSelected(ItemModel item) {
    if (_selectedFavourites.contains(item)) {
      ...
      _itemQuantities.remove(item.productId); // Remove quantity if item is deselected
    } else {
      _selectedFavourites.add(item);
      if (_itemQuantities[item.productId] == null) {
        _itemQuantities[item.productId] = 1; // Initialize quantity to 1 if not already set
      }
    }
    notifyListeners();
  }
  void removeFromFavourites(ItemModel item) {
    _selectedFavourites.remove(item);
    _itemQuantities.remove(item.productId); // Optionally remove quantity as well
  ...
  void updateQuantity(ItemModel item, int newQuantity) {
    ...
    if (_selectedFavourites.contains(item) && newQuantity >= 1) {
      _itemQuantities[item.productId] = newQuantity;  // Update quantity based on product ID
  ...
  String getTotalPrice() {
    ...
    for (ItemModel item in _selectedFavourites) {
      String priceStr = item.unitPrice.replaceAll('', '').replaceAll('\$', ''); // Remove currency symbol
      double price = double.tryParse(priceStr) ?? 0; // Use 0 as default if parsing fails
